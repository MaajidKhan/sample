...
RUN pip3 install git-lfs
RUN git clone https://github.com/MaajidKhan/sample.git && cd sample && mv openvino-ep.py ../ && cd ../ && rm -rf sample
RUN git clone https://github.com/onnx/models.git  && cd models && git lfs pull --include="vision/classification/alexnet/model/bvlcalexnet-9.onnx" --exclude="" && mv vision/classification/alexnet/model/bvlcalexnet-9.onnx ../ && cd ../ && rm -rf models
